...
由于 Vercel 默认域名 `*.vercel.app` 在国内访问不稳定，绑定自定义域名是最佳解决方案。

### 1. 添加域名
1. 在 Vercel 项目控制面板点击 **"Settings"** -> **"Domains"**。
2. 在输入框中输入你的域名（如 `eat.yourdomain.com`），点击 **"Add"**。

### 2. 配置 DNS（关键步骤 ✨）
前往你的域名服务商，添加一条新的解析记录。

> [!TIP]
> 推荐使用 `cname-china.vercel-dns.com` 而非官方默认值，可以利用 Vercel 专门为中国网络优化的接入入口，显著提升访问稳定性。

#### 以腾讯云（DNSPod）操作为例：
1. 登录 [腾讯云 DNSPod 控制台](https://console.cloud.tencent.com/cns)。
2. 在域名列表中点击需要配置的域名。
3. 点击 **"添加记录"**：
   - **主机记录**：填写你的前缀（如 `eat`；若直接用主域名，填写 `@`）。
   - **记录类型**：选择 `CNAME`。
   - **记录值**：填写 `cname-china.vercel-dns.com`。
   - **TTL**：保持默认即可。
4. 点击 **"确认"**，等待解析生效。
...
### 4. 在国内无法打开默认链接？
- 这是由于网络环境限制导致的。请参考上方的 **“绑定自定义域名”** 章节，或者考虑使用更适合国内环境的平台（如 Zeabur）。
